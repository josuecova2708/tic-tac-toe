// SVG definitions
const svgX = `<?xml version="1.0" encoding="UTF-8"?>
...
const svgO = `<svg width="50" height="50" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <circle cx="12" cy="12" r="10" fill="none" stroke="#F2B237" stroke-width="2"/>
</svg>`;
...
const scoreX = document.querySelector('.x span');
const scoreTies = document.querySelector('.t span');
const scoreCPU = document.querySelector('.cpu span');
...
const newGameCPUButton = document.querySelector('.ngc');
const newGamePlayerButton = document.querySelector('.ngp');
...
function initGame() {
    ...
    chooseX.addEventListener('click', () => selectSymbol('X'));
    chooseO.addEventListener('click', () => selectSymbol('O'));
    newGameCPUButton.addEventListener('click', () => startNewGame('cpu'));
    newGamePlayerButton.addEventListener('click', () => startNewGame('player'));
}
// Seleccionar símbolo del jugador
function selectSymbol(symbol) {
    playerSymbol = symbol;
    cpuSymbol = symbol === 'X' ? 'O' : 'X';
    modalInicio.style.display = 'none';
}
// Iniciar nuevo juego contra CPU
...
function announceWinner(winner) {
    ...
    modalContent.innerHTML = `
        <h2>${winner === playerSymbol ? 'YOU WON!' : 'CPU WON!'}</h2>
        <span>${winner} TAKES THE ROUND</span>
...
function updateScores() {
    scoreX.innerHTML = `X<br>${scores.X}`;
    scoreTies.innerHTML = `TIES<br>${scores.Ties}`;
    scoreCPU.innerHTML = `O<br>${scores.O}`;
}

// Actualizar display de turno
function updateTurnDisplay() {
    turnButton.innerHTML = `${currentPlayer} TURN`;
}
